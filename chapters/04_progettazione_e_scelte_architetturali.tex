...
I dati attraversano la pipeline dei moduli tramite diversi cambi di protocollo affinché ogni modulo possa interpretare le richieste in modo corretto.
\begin{itemize}
...
A questo punto è necessario introdurre lo stack tecnologico di implementazione e di ambiente:
\begin{itemize}
    \item \textbf{Proxmox VE:} La scelta dell'hypervisor è ricaduta su una soluzione open-source ampiamente utilizzata sia in ambito enterprise che in quello consumer, corredata dall'esposizione di una REST API solida e ben documentata. Quest'ultimo rappresenta un requisito fondamentale affinché il middleware possa orchestrare le risorse.

    \item \textbf{Python 3.11+:} Per quanto riguarda il linguaggio di implementazione del middleware, è stato reputato appropriato l'uso di Python (versione 3.11+) per le qualità che lo contraddistinguono come lo standard \textit{de facto} per lo sviluppo nell'ambito dell'\textit{AI} e dell'automazione di sistema. La sua adozione ha permesso di sfruttare un ecosistema di librerie ormai maturo per il networking (richieste SSH, HTTP), in particolare:
    \begin{itemize}
        \item \textit{proxmoxer:} Questo wrapper traduce l'albero degli endpoint REST di Proxmox in chiamate a oggetti Python dinamici, evitando la gestione manuale degli header dei pacchetti, dei token di autenticazione e del parsing JSON.
        \item \textit{paramiko:} L'uso di questa libreria rappresenta una soluzione architetturale per aggirare i limiti delle REST API esposte, permettendo l'esecuzione di comandi da riga di comando (\textit{CLI}) tramite il protocollo SSHv2, utile all'instaurazione di tunnel criptati host-to-guest.
    \end{itemize}
    
    \item \textbf{Anthropic MCP SDK:} Come già discusso nei capitoli precedenti, l'esposizione dei comandi Python all'LLM è delegata al protocollo MCP di Anthropic tramite l'adozione dell'SDK ufficiale. Questo gestisce nativamente il ciclo di vita del server MCP, la negoziazione delle capacità, l'invocazione dei tool e la formattazione in JSON-RPC del flusso di comunicazione.
    
    \item \textbf{Claude Client:} Le interazioni Utente-LLM avvengono tramite il client ufficiale di Claude per desktop, che permette l'iniezione del server MCP tramite un file locale di configurazione JSON.
\end{itemize}
