...
\subsection{Attori}
\begin{itemize}
    \item \textbf{Hypervisor bare-metal Proxmox VE:} Ambiente di virtualizzazione installato direttamente sull'hardware con cui si ha necessità di interagire.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Attori Coinvolti:} Utente (iniziatore), Client LLM (mediatore), Hypervisor Proxmox VE (sistema secondario).
        \item \textbf{Precondizioni:} Il Server MCP è attivo, in ascolto sul canale di trasporto designato e autenticato con successo verso l'API REST di Proxmox VE tramite token segregato.
        ...
        \begin{enumerate}
            ...
            \item Proxmox VE elabora la richiesta ed emette la risposta con codice HTTP 200 contenente le metriche grezze.
            \begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \end{itemize}
            \item Il Server MCP invoca gli endpoint REST dell'Hypervisor necessari a soddisfare la richiesta.
            \item Proxmox VE elabora la richiesta ed emette la risposta con codice HTTP 200 contenente l'esito della richiesta.
            \begin{itemize}
                \item Se l'hypervisor risulta irraggiungibile viene restituito un errore di Connection Timeout.
            \end{itemize}
    ...
    L'Utente desidera l'eliminazione di una certa istanza di virtualizzazione.
    \begin{itemize}
        \item \textbf{Attori Coinvolti:} Utente (iniziatore), Client LLM (mediatore), Hypervisor Proxmox VE (sistema secondario).
        \item \textbf{Precondizioni:} Il Server MCP è attivo, in ascolto sul canale di trasporto designato e autenticato con successo verso l'API REST di Proxmox VE tramite token segregato.
        \item \textbf{Flusso Principale degli Eventi:}
        \begin{enumerate}
            ...
            \item Il Server sospende l'esecuzione dell'API verso Proxmox e restituisce all'LLM una richiesta formale di validazione, impedendo l'azione silente (\textit{RF6}).
